...
BEGIN
    ...
    VALUES (p_staff_id, p_month, p_year, v_base_salary, v_deduction_amount, GREATEST(0, v_base_salary - v_deduction_amount), 'pending')
...
-- ======================================
CREATE TABLE IF NOT EXISTS active_persons (
  id uuid,
...
CREATE TABLE IF NOT EXISTS active_students (
  id uuid,
  person_id uuid,
  admission_no character varying(30),
  admission_date date,
...
CREATE TABLE IF NOT EXISTS driver_devices (
  ...
  last_active timestamp with time zone,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT driver_devices_pkey PRIMARY KEY (driver_id)
);
...
CREATE TABLE IF NOT EXISTS driver_heartbeat (
  ...
  CONSTRAINT driver_heartbeat_pkey PRIMARY KEY (driver_id)
);
...
CREATE TABLE IF NOT EXISTS timetable_entries (
  ...
  CONSTRAINT timetable_entries_class_section_id_period_id_day_of_week_key UNIQUE (class_section_id, period_id, day_of_week) -- TODO: VERIFY,
...
CREATE TABLE IF NOT EXISTS ui_route_permissions (
  ...
  CONSTRAINT ui_route_permissions_route_key_key UNIQUE (route_key) -- TODO: VERIFY
);
