...
async function processBatch(batchId, targetMap, month, adminId) {
  ...
  // FIX 6: Pre-fetch ALL balances in a single bulk query instead of N+1
  const allStudentIds = targets.map((t) => t.student_id);
  const balanceRows = await sql`
        SELECT student_id, COALESCE(SUM(amount_due - discount - amount_paid), 0) as balance
        FROM student_fees
        WHERE student_id = ANY(${allStudentIds})
  ...
  for (let i = 0; i < targets.length; i += CHUNK_SIZE) {
    ...
    const promises = chunk.map(async (student) => {
      try {
        ...
        if (amountDue <= 0) return; // Skip if no dues
      ...
      if (type === 'FEE_REMINDER') {
        ...
        for (let i = 0; i < recipients.length; i += CHUNK_SIZE) {
          ...
          await Promise.all(chunk.map(async (r) => {
            ...
            const response = await sendNotificationToUsers([r.id], type, payload, { senderId: adminId, batchId: batch.id, role: 'test' });
        ...
        const response = await sendNotificationToUsers(userIds, type, payload, { senderId: adminId, batchId: batch.id, role: 'test' });
...
router.post('/diary/send-all', requirePermission('diary.manage'), asyncHandler(async (req, res) => {
  ...
  (async () => {
    try {
      const userIds = recipients.map((r) => r.id);
      const response = await sendNotificationToUsers(userIds, 'DIARY_UPDATED', { message: `New diary entry posted for ${date}.` }, { senderId: adminId, batchId: batch.id, role: 'student' });
...
router.post('/results/send-all', requirePermission('results.publish'), asyncHandler(async (req, res) => {
  ...
  (async () => {
    try {
      const userIds = recipients.map((r) => r.id);
      const response = await sendNotificationToUsers(userIds, 'RESULT_RELEASED', { message: `Results for ${exam.name} have been published.` }, { senderId: adminId, batchId: batch.id, role: 'student' });
      const sentCount = response?.successCount || 0;
      const failureCount = response?.failureCount || 0;
...
router.post('/notices/retrigger', requirePermission('notices.manage'), asyncHandler(async (req, res) => {
  ...
  (async () => {
    try {
      ...
      const response = await sendNotificationToUsers(userIds, 'NOTICE_ADMIN_STUDENT', { message: `Important Notice: ${notice.title} ` }, { senderId: adminId, batchId: batch.id, role: 'all' });
      const sentCount = response?.successCount || 0;
      const failureCount = response?.failureCount || 0;
 ...
 * Triggers a push notification to a user regarding their access request.
 */
router.post('/access-response', requirePermission('dashboard.view'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, sentCount: response?.successCount || 0, failureCount: response?.failureCount || 0 });
  } catch (err) {
